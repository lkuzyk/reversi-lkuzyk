...
socket.on('join_room_response', (payload) => {
    ...
    let newHTML = '<p class=\'join_room_response\'>'+ payload.username + ' joined the chatroom. </br> (There are '+ payload.count +' users in this room)</p>'; 
    let newNode = $(newHTML);
    newNode.hide();
...
socket.on('player_disconnected', (payload) => {
    ...
    payload.username+' left the Lobby. (There are '+payload.count+' users in this room)</p>'; 
    let newNode = $(newHTML);
    newNode.hide();
    $('#messages').prepend(newNode);
    newNode.show("fade", 500);
    ...
    for (let row = 0; row < 8; row++) {
        for (let column = 0; column < 8; column++) {
            ...
            if (old_board[row][column] !== board[row][column]) {
                ...
                // Update image with selected graphic
                const t = Date.now();
                $('#' + row + '_' + column).html('<img class="img-fluid" src="assets/images/' + graphic + '?time=' + t + '" alt="' + altTag + '"/>');

                // Remove previous click event to prevent accumulation of handlers
                $('#' + row + '_' + column).off('click');

                // Check if the board space is empty
                if (board[row][column] === ' ') {
                    $('#' + row + '_' + column).addClass('hovered_over');
                    $('#' + row + '_' + column).click(handleClick(row, column));
                } else {
